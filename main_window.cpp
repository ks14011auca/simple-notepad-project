...
void main_window::setup_format_menu()
{
    ...
    for (const auto& transform : transforms) {
        const auto* transform_ptr = transform.get();
        const auto* action = text_case_menu->addAction(QString::fromStdString(transform_ptr->name()));
        connect(action, &QAction::triggered, this, [this, transform_ptr] {
...
void main_window::show_find_replace_dialog()
{
    if (!find_replace_dlg) {
        ...
        connect(find_replace_ui->close_button, &QPushButton::clicked,
            find_replace_dlg, [this] {
                find_replace_dlg->hide();
            });
    }
...
void main_window::show_word_frequency()
{
    ...
    for (int i = 0; i < static_cast<int>(sorted_freq.size()); ++i) {
        const auto& [word_text, count] = sorted_freq[i];

        ui.frequency_table->setItem(i, 0,
            new QTableWidgetItem(QString::fromStdString(word_text)));
